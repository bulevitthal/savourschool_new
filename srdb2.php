...
function getDBSiteURL($host,$u,$p,$n) {
	$connection = @mysql_connect( $host, $u, $p );
	global $tables;
	$prefix_arr = explode('_',$tables[0],-1);
	$db_prefix = implode($prefix_arr);
	
	@mysql_select_db( $n, $connection );
	//$all_tables_mysql = @mysql_query( 'SHOW TABLES', $connection );
	$result=mysql_query( 'SELECT option_value FROM '.$db_prefix.'_options WHERE `option_id` =1', $connection );
	...
	case 4:
		...
		<form action="<?php icit_srdb_form_action( ); ?>" method="post">
			<fieldset>
				...
				if(empty($srch))		
				{
					$srch= getDBSiteURL($host,$user,$pass,$data);
