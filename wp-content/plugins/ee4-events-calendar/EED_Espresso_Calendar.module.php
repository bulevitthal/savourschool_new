	...
	private function _get_filter_html($ee_calendar_js_options = array()){
		...
		if ( ! $ee_calendar_js_options['widget'] ) {
			...
			<div id="espresso-category-legend">
				...
				echo '<li id="ee-category-li-all" class="ee_filter_show_all">
						<span class="ee-category"><a href="">View All</a></span>
					</li>';
			...
			if ( $datetime instanceof EE_Datetime ) {
				...
				if ( ! $this->config()->display->disable_categories) {
					...
					if ( $categories ) {
						...
						if ( $this->config()->display->enable_cat_classes ) {
							...
							$difficulty = get_field('class_difficulty_level', $event->ID());
							$calendar_datetime->add_classname($primary_cat->slug());
							$calendar_datetime->add_classname($difficulty);
							}
						}
				...
				if ( $this->config()->tooltip->show ) {
					...
					$tooltip_html .= ! $pswrd_required && $startTime && $this->config()->time->show
						? '<p class="time_cal_qtip">' . __('', 'event_espresso') . $startTime . ' - ' . $endTime . '</p>'
						: '';
			        ...
			        $ticket = $wpdb->get_results( $wpdb->prepare( $sql, $datetime->ID() ));

		        	$sql = "SELECT TKT_price ";
			        $sql .= "FROM {$wpdb->prefix}esp_ticket ";
			        $sql .= "WHERE TKT_ID = %d";

			        $ticket_info = $wpdb->get_results( $wpdb->prepare( $sql, $ticket[0]->TKT_ID ));

					$tooltip_html .= ' <p class="class_cost">$'.number_format((float)$ticket_info[0]->TKT_price, 2, '.', ''). '</p>';
